...
Create a sequence of integers from ```start``` to ```end```, similarly to doing
in R ```seq(start, to, by = 1)```, where ```start``` and ```end``` are integers. 

Find the number of elements of the integer array $y$ such that 
$lb \leq y \leq ub$, similarly to doing in R ```sum(y >= 4 & y <= 6)``` or ```sum(dplyr::between(y, 4, 6))```.

Find the elements of the integer array $y$ such that $lb \leq y \leq ub$, 
similarly to doing in R ```y[y >= 4 & y <= 6]```.

Find the elements of the vector $y$ whose index is $\geq lb$ and whose index is 
$\leq ub$, similarly to doing in R ```y[idx >= 4 & idx <= 6]```

Generate a random number from a normal distribution that is truncated below at
```lb``` (often this is 0).

in @sec-statistical-model
